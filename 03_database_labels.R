...
# Outputs:
#   - Updates SQLite database with paragraph_label_table and keyword lookup tables
################################################################################
...
## Load Keyword Tables ----
clause_type_keywords <- fread(here("data", "clause_type_keywords.csv"), colClasses = "character")
iucn_l2_keywords <- fread(here("data", "iucn_l2_keywords.csv"), colClasses = "character")
salmon_scope_keywords <- fread(here("data", "salmon_scope_keywords.csv"), colClasses = "character")
governance_keywords <- fread(here("data", "governance_keywords.csv"), colClasses = "character")
management_domain_threat_table <- fread(here("data", "management_domain_threat_table.csv"), colClasses = "character")
...
## Matching Function (Vectorized for Performance) ----
match_keywords <- function(tokens, paragraph_id, keyword_list, label_type, value_column = NULL) {
  # Convert underscores to spaces in tokens for matching
  cleaned_tokens <- gsub("_", " ", tokens)
  
  # Vectorized keyword matching - check which keywords are in the token list
  matched <- keyword_list$keyword %in% cleaned_tokens
  
  if (sum(matched) == 0) {
    return(data.table())
  }
  
  matched_keywords <- keyword_list[matched]
  
  if (!is.null(value_column) && value_column %in% names(matched_keywords)) {
    label_values <- matched_keywords[[value_column]]
  } else {
    label_values <- rep(NA_character_, nrow(matched_keywords))
  }
...
batch_size <- 1000
n_paragraphs <- length(paragraph_table$paragraph_id)
n_batches <- ceiling(n_paragraphs / batch_size)

paragraph_label_list <- list()

for (batch in 1:n_batches) {
  start_idx <- (batch - 1) * batch_size + 1
  end_idx <- min(batch * batch_size, n_paragraphs)
  
  batch_results <- mapply(function(tokens, pid) {
    rbindlist(list(
  ...
  }, token_list[start_idx:end_idx], paragraph_table$paragraph_id[start_idx:end_idx], SIMPLIFY = FALSE)
  
  paragraph_label_list[[batch]] <- rbindlist(batch_results)
  
  if (batch %% 10 == 0 || batch == n_batches) {
    cat(sprintf("  Processed %d/%d paragraphs (%.1f%%)\n", end_idx, n_paragraphs, 100 * end_idx / n_paragraphs))
  }
}

paragraph_label_table <- rbindlist(paragraph_label_list)
...
for (batch in 1:n_batches) {
  ...
  }, heading_token_list[start_idx:end_idx], paragraph_table$paragraph_id[start_idx:end_idx], SIMPLIFY = FALSE)
  
  heading_label_list[[batch]] <- rbindlist(batch_results)
  
  if (batch %% 10 == 0 || batch == n_batches) {
    cat(sprintf("  Processed %d/%d headings (%.1f%%)\n", end_idx, n_paragraphs, 100 * end_idx / n_paragraphs))
  }
}

heading_label_table <- rbindlist(heading_label_list)
